...
- [x] CSV et Excel (`.xlsx`) sont acceptés
- [x] Un template CSV et Excel sont téléchargeables
- [x] L'étape de mapping des colonnes est affichée après upload avec auto-détection
- [x] L'utilisateur peut désigner la colonne "identifiant unique" pendant le mapping
- [x] Le fichier est parsé côté client et prévisualisé avant import avec le diff complet
- [x] Les lignes d'un même film (même identifiant) sont regroupées en 1 film + N zones de prix
- [x] Films dans le fichier mais absents en base → créés
- [x] Films dans le fichier et présents en base → mis à jour (données + remplacement complet des prix)
- [x] Films absents du fichier et présents en base → archivés (`retired`)
- [x] Films avec erreurs → ignorés (ni mis à jour, ni archivés)
- [x] La confirmation affiche la liste des films à archiver avec avertissement irréversible
- [x] Si des films sont à archiver, la confirmation demande de saisir `CONFIRMER`
- [x] Les prix sont convertis en centimes à l'ingestion
- [x] L'enrichissement TMDB est lancé en arrière-plan pour les nouveaux films
- [x] Les nouveaux films apparaissent immédiatement avec le badge `pending`
- [x] Un rapport post-import affiche le nombre de créations / mises à jour / archivages / erreurs
- [x] Fichiers > 500 lignes ou > 10 Mo sont rejetés avec un message clair
...
- [x] Les erreurs bloquantes empêchent l'import de la ligne concernée
- [x] Les warnings sont affichés mais n'empêchent pas l'import
- [x] La colonne et la ligne de chaque erreur sont indiquées précisément
- [x] La validation serveur rejette également les données invalides (double sécurité)
...
- [x] La recherche TMDB est déclenchée automatiquement en temps réel lors de la saisie du titre
- [x] L'ayant droit peut créer un film sans interagir avec TMDB (`tmdbMatchStatus = "no_match"`)
- [x] Au moins une zone de prix est requise à la création
- [x] Si un résultat TMDB est sélectionné, l'enrichissement est effectué synchroniquement à la sauvegarde
- [x] Si TMDB est indisponible après sélection, la création réussit avec `tmdbMatchStatus = "pending"`
...
- [x] À la création manuelle avec sélection TMDB, les données TMDB sont peuplées synchroniquement
- [x] À l'import, les films apparaissent immédiatement avec le badge "Enrichissement en cours" (`pending`)
- [x] L'enrichissement asynchrone met à jour `tmdbMatchStatus` pour tous les films du batch
- [x] Le badge de statut TMDB est visible sur la liste de films
- [x] Si TMDB est indisponible pour un film lors du batch, ce film reste `pending` et l'enrichissement continue pour les suivants
...
- [x] Les données TMDB sont affichées en lecture sur la fiche film
- [x] L'ayant droit peut modifier manuellement les champs TMDB
- [x] Le bouton "Resynchroniser" relance un enrichissement TMDB complet
- [x] La désassociation passe `tmdbMatchStatus` à `"no_match"` et efface les données TMDB
