...
interface RightsHolderListProps {
...
export function RightsHolderList({
  initialAccounts,
  initialTotal,
  defaultCommissionRate,
}: RightsHolderListProps) {
  ...
  // Commission dialog
  const [commissionTarget, setCommissionTarget] = useState<AdminAccountRow | null>(null);
  ...
  function formatCommission(rate: string | null): string {
    if (!rate) return `${(defaultCommissionRate * 100).toFixed(0)}% (${t("defaultRate")})`;
  ...
  const fetchAccounts = useCallback(
    async (search: string, page: number) => {
      setLoading(true);
      try {
        const result = await getRightsHoldersPaginated({
  ...
  async function handleSuspendToggle() {
    ...
    try {
      ...
      const result = isSuspended
        ? await reactivateRightsHolderAction(suspendTarget.id)
        : await suspendRightsHolderAction(suspendTarget.id);
  ...
  async function handleCommissionUpdate() {
    ...
    try {
      ...
      const result = await updateCommissionRateAction(commissionTarget.id, rate);
      if ("error" in result) {
        toast.error(result.error);
      } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <Table className="table-fixed">
        <TableHeader>
          <TableRow className="border-border/40">
            ...
            <TableHead className="w-[10%]">{t("columns.films")}</TableHead>
            <TableHead className="w-[12%]">{t("columns.commission")}</TableHead>
            <TableHead className="w-[13%]">{t("columns.stripe")}</TableHead>
            <TableHead className="w-[10%]">{t("columns.volume")}</TableHead>
            ...
            accounts.map((account) => (
              <TableRow key={account.id} className="border-b border-border/40">
                <TableCell className="font-medium">
                  <Link
                    href={`/${locale}/admin/rights-holders/${account.id}`}
                ...
                <TableCell>{account.activeFilmCount}</TableCell>
                <TableCell className="text-sm">
                  {formatCommission(account.commissionRate)}
                </TableCell>
                <TableCell>
                  <Badge
                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem
                        onClick={() => router.push(`/${locale}/admin/rights-holders/${account.id}`)}
                      >
                        <Eye className="mr-2 size-4" />
                        {t("view")}
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => setCommissionTarget(account)}>
      ...
      {/* Commission dialog */}
      <Dialog open={!!commissionTarget} onOpenChange={() => setCommissionTarget(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("commissionDialog.title")}</DialogTitle>
            <DialogDescription>
              {t("commissionDialog.description", {
                name: commissionTarget?.companyName ?? "",
                default: `${(defaultCommissionRate * 100).toFixed(0)}%`,
              })}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setCommissionTarget(null)}>
              {t("cancel")}
            </Button>
            <Button onClick={handleCommissionUpdate} disabled={updatingCommission}>
              {updatingCommission && <Loader2 className="mr-2 size-4 animate-spin" />}
