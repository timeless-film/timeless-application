...
import type { TopFilm } from "@/lib/services/rights-holder-dashboard-service";

interface RightsHolderTopFilmsProps {
  data: TopFilm[];
}

export function RightsHolderTopFilms({ data }: RightsHolderTopFilmsProps) {
  const t = useTranslations("rightsHolderDashboard.topFilms");
              ...
              <div key={film.filmId} className="flex items-center gap-3 py-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-muted-foreground text-xs">
                    {film.orderCount} {t("sales")} ·{" "}
                    {formatAmount(film.totalRevenue, "EUR", locale)}
                  </p>
                </div>
              </div>
            ))}
